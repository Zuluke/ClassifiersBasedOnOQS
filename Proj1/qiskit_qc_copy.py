...
def circuit_model(data,contador,w,counter,qubits,N_qubits,N_features,model=None,folder=None,N_qubits_tgt=None,printar_cirq=False,N_layers=None):
    ...
    if model=='IQC':
        ...
        for i, instruction in enumerate(tqc.data):
            names.append(instruction.operation.name)
        ...
        return qc,names, get_negativity(rho,[2, len(X_new)])
        ...
        return qc,names, get_negativity(rho,[2, N_features])
...
def statistical_qc(N_samples,N_features,simulation_samples,model=None,folder=None,normalization=False,N_qubits_tgt=None,esfera=False,N_layers=None):
    ...
    if esfera==True:
        if normalization:
            ...
            names,neg_lista=esfera_bloch(X_df,w_df,qubits,N_qubits,N_features,counter=counter,model=model,folder=folder,N_qubits_tgt=N_qubits_tgt,N_layers=N_layers)
            return names, neg_lista
    else:
        X_df=normalize_model(X_df,model=model,normalize_col=True,normalize_lin=False)
        names,neg_lista,express=expressibility(X=X_df,weights=w_df,qubits=qubits,N_qubits=N_qubits,
                                               N_features=N_features,simulation_samples=simulation_samples,
                                               counter=counter,model=model,folder=folder,N_qubits_tgt=N_qubits_tgt,
                                               N_layers=N_layers)
        ...
        unique_G_list=np.unique_counts(lens)

        lista=size_divide(names)
        return names, neg_lista, express
...
def expressibility(X, weights, qubits, N_qubits, N_features, counter, simulation_samples, model=None, folder=None, norma=None, N_qubits_tgt=None, N_layers=None):
    ...
    negativity=[]
    for k in range(len(X)):
        _,names,neg=circuit_model(X[k],k,weights[k], counter, qubits, N_qubits, N_features,folder=folder,model=model,N_qubits_tgt=N_qubits_tgt,N_layers=N_layers)
    ...
    return names_list,negativity,express
